...
export function apply(ctx, config) {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    if (routerMode === 'standard') {
      ...
      if (isSubagent) {
        ...
        const missionNote = { name: 'mission-note', order: 1, text: 'Long missions: FIRST load mission-protocol and task-profile skills, call mission_start, and write task-profile.md before substantive research/solving. Delegate substantive work to role subagents. After dispatching background subagents, prefer setting a schedule_reminder and ending the turn over repeated waiting.' }
  ...
  const MISSION_GUIDE =
    '\nLong-Run Captain: this is a long-running mission. Your FIRST action must be to load mission-protocol and task-profile skills, call mission_start, and write task-profile.md/plan.md before substantive solving. Delegate substantive modeling/implementation/writing tasks to role subagents; do not solve the whole problem yourself.'
