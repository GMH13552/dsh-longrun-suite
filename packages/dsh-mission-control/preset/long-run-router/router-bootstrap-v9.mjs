...
export function apply(ctx, config) {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    if (routerMode === 'standard') {
      ...
      if (isSubagent) {
        // Use the same minimal router persona as the root. The child's role is
        // carried by the task prompt, so keeping the role persona here pulls
        // subagents into solitary "Let me" doer mode.
        sections = [
