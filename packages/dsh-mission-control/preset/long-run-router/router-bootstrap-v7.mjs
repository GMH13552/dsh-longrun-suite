...
export function apply(ctx, config) {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    if (routerMode === 'standard') {
      ...
      if (isSubagent) {
        ...
        // Do NOT put mission instructions in the first-turn minimal system:
        // that confused flash into looking for tools that were not exposed yet.
        // After the first durable tool/call the full catalog exists, so inject
        // the mission-operating note only at that point.
        const missionNote = hasToolCall
          ? { name: 'mission-note', order: 1, text: 'Full tool catalog is now available. If this is a long-running or complex multi-step task and you have not started a mission, use the `skill` tool to load mission-protocol and task-profile, then call mission_start and write task-profile.md before continuing substantive work. Delegate substantive work to role subagents. Prefer setting a schedule_reminder and ending the turn over repeated waiting.' }
          : null
        const baseSections = planSection
          ? [planSection, { name: 'router-persona', text: persona, order: 0 }]
          : [{ name: 'router-persona', text: persona, order: 0 }]
        sections = missionNote ? [...baseSections, missionNote] : baseSections
      }
  ...
  const MISSION_GUIDE =
    '\nLong-Run Captain: this is a long-running or complex multi-step task. If you have not started a mission, use the `skill` tool for mission-protocol and task-profile skills, then call mission_start and write task-profile.md/plan.md before substantive solving. Do not search for skill files on disk. Delegate substantive modeling/implementation/writing tasks to role subagents; do not solve the whole problem yourself.'
