...
export function resolveVerifierConfig(config = {}) {
  ...
  const resolved = {
    ...
    groundTruthNote: config.groundTruthNote === false ? '' : (config.groundTruthNote ?? DEFAULT_GROUND_TRUTH_NOTE),
    ...
    rollout: {
      ...
      criteria: config.rollout?.criteria === undefined || (Array.isArray(config.rollout.criteria) && config.rollout.criteria.length === 0)
        ? ROLLOUT_CRITERIA
        : resolveCriteria(config.rollout.criteria),
    },
  }
...
/** mulberry32: a tiny seeded PRNG for reproducible ring passes. */
...
/** Fisher-Yates shuffle (fresh array); seeded when `random` is given. */
...
export class VerifierEngine {
   ...
   */
  async comparePair(task, candidateA, candidateB, { signal, alternate = true, criteria } = {}) {
    const tags = ['score_A', 'score_B']
    const system = verifierSystemPrompt(this.config.granularity, tags)
    const jobs = []
    for (const criterion of criteria ?? this.config.criteria) {
    ...
    const samples = await mapLimit(jobs, this.config.concurrency, async job => {
      ...
      const parsed = await this.#sample(system, compareUserPrompt(task, first, second, job.criterion, this.config.groundTruthNote), tags, signal)
  ...
  async score(task, candidate, { signal } = {}) {
    ...
    const perCriterion = await mapLimit(this.config.criteria, 1, async criterion => {
      const userText = scoreUserPrompt(task, candidate, criterion, this.config.groundTruthNote)
   ...
   */
  async select(task, candidates, { signal, criteria } = {}) {
    if (!Array.isArray(candidates) || candidates.length < 2) fail('select needs at least 2 candidates')
    const n = candidates.length

    // Majority-voting shortcut (reference behavior): when a strict majority
    // of candidates are byte-identical, the tournament is redundant.
    if (this.config.majorityVoting) {
      const counts = new Map()
      for (const candidate of candidates) counts.set(candidate, (counts.get(candidate) ?? 0) + 1)
      const [majorityText, majorityCount] = [...counts.entries()].reduce((best, entry) => (entry[1] > best[1] ? entry : best))
      if (majorityCount > n / 2) {
        const bestIndex = candidates.indexOf(majorityText)
        const rewards = candidates.map(candidate => (candidate === majorityText ? 1 : 0))
        return { bestIndex, bestReward: 1, rewards, ringScores: rewards, pivots: [bestIndex], pairs: [], majority: majorityCount }
      }
    }
    ...
    // 1. Ring pass over a random Hamiltonian cycle. Like the reference
    //    implementation, odd repetitions also swap A/B slots within every
    //    directed comparison, so positional bias cancels both around the
    //    cycle and inside each comparison.
    const random = Number.isInteger(this.config.seed) ? mulberry32(this.config.seed) : Math.random
    const cycle = shuffle(Array.from({ length: n }, (_, i) => i), random)
    const edges = cycle.map((from, idx) => [from, cycle[(idx + 1) % n]])
    await scorePairs(edges, true)

    // 2. Pivot selection by ring-pass mean PREFERENCE w_i / c_i (reference
    //    pivot_tournament.select_pivots), not by mean raw reward.
    const ringWin = new Array(n).fill(0)
    const ringCount = new Array(n).fill(0)
    for (const { i, j, rewardI, rewardJ } of pairResults.values()) {
      ringWin[i] += sigmoid(rewardI - rewardJ)
      ringWin[j] += sigmoid(rewardJ - rewardI)
      ringCount[i] += 1
      ringCount[j] += 1
    }
    const ringScores = ringWin.map((w, i) => (ringCount[i] > 0 ? w / ringCount[i] : 0))
  ...
  async track(task, trajectory, { signal } = {}) {
    ...
    const system = trackSystemPrompt(this.config.granularity)
    const prefixes = trajectory.map((_, i) => trajectory.slice(0, i + 1))
    const progress = await mapLimit(prefixes, 1, async steps => {
      const userText = trackUserPrompt(task, steps, this.config.groundTruthNote)
      const reps = Array.from({ length: this.config.repetitions })
      const samples = await mapLimit(reps, this.config.concurrency, () =>
        this.#sample(system, userText, ['score'], signal).then(parsed => parsed.score))
      const valid = samples.filter(v => v !== null)
